...
class auth_wizard(osv.osv_memory):
    ...
    def button_execute(self, cr, uid, ids, context=None):
        ...
        seqtype_obj = self.pool.get('ir.sequence.type')
        ...
        for res in self.browse(cr, uid, ids, context=context):
            if not aut_obj.search(cr, uid, [('number','=',res['number']),]):
                ...
                for dt in res['type_document_wizard_ids']:
                    ...
                    # armamos el tipo de secuencia, debe crearse antes de crearse la autorizacion
                    # debe verificarse si ya existe este tipo
                    # Hay que tomar en cuenta que el codigo debe ser unico, por eso se ocupa el numero de autorizacion

                    vals_seqtype = {
                        'name': 'Autorizacion SRI',
                        'code': code,
                                  }
                    
                    seq_type_id = seqtype_obj.search(cr, uid, [('code','=',vals_seqtype['code'])])
                    
                    #seq_type_id = seqtype_obj.search(cr, uid, [('name','=', vals_seqtype['name'] ),('code','=',vals_seqtype['code'])])
                    
                    # si no existe creada un tipo de secuencia, creamos una
                    if not seq_type_id:
                        seq_type_id = seqtype_obj.create(cr, uid, vals_seqtype, context=context)
...
class auth_wizard_line(osv.osv_memory):
    ...
    _columns={
              ...
              'state':fields.selection(_get_name,  'state', required=True, readonly=False), #antes: readonly=True
