        ...
        for inv in inv_obj.browse(cr, uid, ids):
            if inv.type in ["out_invoice"]:
                ...
                if invoice.get('automatic', False):
                    new_number = doc_obj.get_next_value_secuence(cr, uid, 'invoice', False, inv.company_id.id, inv.shop_id.id, inv.printer_id.id, 'account.invoice', 'invoice_number_out', context)
                invoice.update({
                    'state': 'draft',
                    'number': False,
